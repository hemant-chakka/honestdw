<!-- Answers Modal -->
<div class="modal fade answersForm" id="answersForm_{$ID}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
			<% if $Top1.getQuestionsAndAnswers($ID,$null,true) %>
    			<% loop $Top1.getQuestionsAndAnswers($ID,$null,true) %>
    				<form method="post" class="QuestionsAndAnswersForm" action="/get-prices/postAnswer">
    					<div class="form-group row">
      						<div class="col-sm-1">
       							Q{$Up.Top1.Subtract($TotalItems,$Pos(0))}
   							</div>
   							<div class="col-sm-8">
       							<% if $Question %>
    					...
    					<div class="form-group row">
  							...
  							<% if $Answer %> 
   								...
   								<% else %>
   									<div class="col-sm-2"><div id="AnswerDocUpload_{$TenderID}_{$Up.Top1.Subtract($TotalItems,$Pos(0))}">Upload</div></div>
   								<% end_if %>
  								<div class="col-sm-3">
   									<% if $AnswerDocumentID %>
   ...
   (function($) {
		$("document").ready(function(){
   			<% loop $Top1.getQuestionsAndAnswers($ID,$null,true) %>
    			...
    			$("#AnswerDocUpload_{$TenderID}_{$Up.Top1.Subtract($TotalItems,$Pos(0))}").uploadFile({
					url:"/get-prices/uploadQandADoc",
					multiple:false,
					dragDrop:false,
					maxFileCount:1,
					fileName:"myfile",
					formData: {"TenderID":$TenderID,"SubmissionID":0,"QaAID":$ID,"DocType":"AnswerDocumentID"}
				});	

<!-- Answers Modal -->
<div class="modal fade answersForm" id="answersForm" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
			<% if $getQuestionsAndAnswers($Tender.ID,$null,true) %>
    			<% loop $getQuestionsAndAnswers($Tender.ID,$null,true) %>
    					...
    					<div class="form-group row">
  							...
  							<% if $Answer %> 
   							...
   							<% else %>
   								...
   								<div class="col-sm-2"><div id="AnswerDocUpload_{$Tender.ID}_{$ID}">Upload</div></div>
   								<div class="col-sm-3">
   ...
   (function($) {
		$("document").ready(function(){
   			<% loop $getQuestionsAndAnswers($Tender.ID,$null,true) %>
    			...
    			$("#AnswerDocUpload_{$Tender.ID}_{$ID}").uploadFile({
					url:"/get-prices/uploadQandADoc",
					multiple:false,
					dragDrop:false,
					maxFileCount:1,
					fileName:"myfile",
					formData: {"TenderID":$Tender.ID,"SubmissionID":$SubmissionID,"QaAID":$ID,"DocType":"AnswerDocumentID"}
				});	
